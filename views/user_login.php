...
if (isset($_POST['login'])) {
    ...
    $stmt = $mysqli->prepare("SELECT login_email, login_password, login_rank, login_user_id FROM login l 
    INNER JOIN user u ON u.user_id = l.login_user_id
    WHERE login_email =? AND login_password =?");
    $stmt->bind_param('ss', $email, $password);
    $stmt->execute(); //execute bind

    $stmt->bind_result($email, $password, $login_rank, $login_user_id);
    $rs = $stmt->fetch();
...
<body class="d-flex flex-column h-100">
    ...
    <main class="flex-shrink-0">
        <div class="container text-center  mt-4">
            ...
            <h4 class="mb-4">iBooking <br> <small>User Login Area</small></h4>
        </div>
        <div class="container">
            <div class="login-box">
                <form method="POST">
